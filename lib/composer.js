...
//
// Require `winston-godot` to add the
// `Godot` transport
//
require('winston-godot');
...
//
// Setup provisioner API, if config is present.
//
composer.use({
  name: 'set-provisioner',
  init: function (done) {
    var provisioner = this.config.get('provisioner');
    this.provisioner = provisioner
      ? conservatory.createClient(provisioner)
      : null;
    done();
  }
});
