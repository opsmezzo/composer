...
composer.router.path(/\/config/, function () {
  ...
  this.get('/servers', function () {
    ...
    async.parallel({
      ...
      //
      roles: function (next) {
        composer.resources.Role.all(function (err, roles) {
      ...
      servers: function (next) {
        composer.resources.Server.all(function (err, servers) {
      ...
      // an index by system and by group and by system. e.g.:
      //
      // {
      ...
      return res.json(200, data.servers.reduce(function (all, server) {
        all.groups[server.group] = all.groups[server.group] || {};

        if (Array.isArray(systems[server.role])) {
          systems[server.role].forEach(function (system) {
            all[system] = all[system] || [];
            all.groups[server.group][system] = all.groups[server.group][system] || [];
            all[system].push(server.addresses);
            all.groups[server.group][system].push(server.addresses);
          });
        }
  ...
  //
  // Get Server Config in Group: `GET /config/servers/group` responds with
  // an index of server addresses in that group by system.
  //
  this.get('/servers/:group', function (group) {
    var res = this.res;
    ...
    async.parallel({
      ...
      //
      servers: function (next) {
        composer.resources.Server.forGroup(group, function (err, servers) {
          return err ? next(err) : next(null, servers);
        });
      }
    }, function (err, data) {
