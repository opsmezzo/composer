...
auth.basicAuth = function () {
  ...
  //
  composer.resources.User.get(credentials[0], function (err, obj) {
    if (err) {
    ...
    //
    var user = obj.properties,
        checksum = hash.md5(credentials[1], user['password-salt']);
    ...
    if (user.username.toLowerCase() === credentials[0] && user.password === checksum) {
      ...
      //
      request.headers.remote_user = request.username = credentials[0];
      request.user = obj;
      callback(null);
    }
    else {
